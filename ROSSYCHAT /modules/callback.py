...
from ROSSYCHAT.database import ROSSY
...
async def cb_handler(_, query: CallbackQuery):
    if query.data == "HELP":
        ...
        if user_status not in [CMS.OWNER, CMS.ADMINISTRATOR]:
            ...
            is_ROSSY = ROSSY.find_one({"chat_id": query.message.chat.id})
            if not is_ROSSY:
                await query.edit_message_text(f"**ᴄʜᴀᴛ-ʙᴏᴛ ᴀʟʀᴇᴀᴅʏ ᴇɴᴀʙʟᴇᴅ.**")
            if is_ROSSY:
                ROSSY.delete_one({"chat_id": query.message.chat.id})
        ...
        if user_status not in [CMS.OWNER, CMS.ADMINISTRATOR]:
            ...
            is_ROSSY = ROSSY.find_one({"chat_id": query.message.chat.id})
            if not is_ROSSY:
                ROSSY.insert_one({"chat_id": query.message.chat.id})
                await query.edit_message_text(
